...
from models.transformer_tuckle import Transformer
...
def train_model(input_params):
    ...
    trace_path = "checkpoints_tuckle/logging/"
    num_epochs = 10
    ...
    for epoch in range(num_epochs):
        ...
        for i, [inputs, targets] in enumerate(dataset):
            ...
            if (i % 100 == 0):
                ...
                time_ = time.time()
            if ((i+1) % 2000 == 0):
                tl.files.save_npz(model.all_weights, name='./checkpoints_tuckle/model.npz')
            if (i == 5000):
