...
def compile_files(raw_dir, raw_files, tag):
  ...
  with gfile.Open(input_compiled_file, mode="w") as input_writer:
    with gfile.Open(target_compiled_file, mode="w") as target_writer:
...
def main(unused_argv):
  ...
  # tf.logging.set_verbosity(print)
...
#   total_shards = _TRAIN_SHARDS
#   train_tfrecord_files = [shard_filename(FLAGS.data_dir, _TRAIN_TAG, n + 1, total_shards)
#     for n in range(total_shards)]
