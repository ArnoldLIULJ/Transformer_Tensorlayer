...
class Transformer_layer(tl.models.Model):
    """
    Transormer model.
...
class EncoderStack(tl.models.Model):
        ...
        for _ in range(params.encoder_num_layers):
            self.sublayers.append([
                SublayerWrapper(SelfAttentionLayer(params.num_heads, params.hidden_size, params.keep_prob),
                                params),
                MultiHeadAttentionLayer(params.num_heads, params.hidden_size, params.keep_prob),
    ...
    def forward(self, inputs, input_mask):
        ...
        for sublayer in self.sublayers:
            inputs_ = sublayer[0](inputs=inputs, mask=input_mask)
            inputs = sublayer[1](inputs, y=inputs_, mask=input_mask)
